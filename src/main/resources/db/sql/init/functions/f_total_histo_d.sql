CREATE function ARTHUS.f_total_histo_d (
				a_idhisto in number,
				a_type in integer
				)
Return number
...
End;
Begin
Select	round( (loc_periode * histo_reval.montant_d), 2 )
Into	loc_reval
From	histo_reval
Where	idhisto = a_idhisto;
...
End;
Begin
Select	Nvl(Sum( round( (loc_periode * nvl(histo_dedu.montant_d, 0)), 2 ) ), 0)
Into	loc_dedu
From	histo_dedu
Where	idhisto = a_idhisto;
...
if ( a_type = 0 ) then
	...
	Begin
	Select
	Nvl(Sum( round( (loc_periode * nvl(histo_dedu.montant_d, 0)), 2 ) ), 0)
	Into	loc_retour
	From	histo_dedu
	Where	idhisto = a_idhisto
