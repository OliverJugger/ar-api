CREATE function ARTHUS.F_CPTA_LIB_ENCAISMT
                (
                I_codope       In Number,
...
if loc_bdx= 1 then
  ...
  BEGIN
    --16/07/2009 SCR : select  substr('0000000' || to_char(numremise),-7,7), compte.journal
    select  substr('0000000' || to_char(numremise),-7,7), NULL
    ...
    union
    select  substr('0000000' || to_char(prelevement.numremise),-7,7), to_char(remise_prelev.date_prelev, 'DD/MM/YY')
