CREATE FUNCTION ARTHUS."F_MONTANT_DU" (
   a_numfact   IN   NUMBER,
   a_codope     IN   NUMBER,   
   a_rappel     IN   NUMBER
)
...
-- $HeadURL: svn://svn2019/arthus/GEREP/trunk/dbschema/ARTHUS/FUNCTIONS/F_MONTANT_DU.sql $:  Chemin
...
BEGIN
  ...
  --	Facture engag̩e dans le processus de relance => renvoie le solde du uniquement si le solde est sup̩rieur au seuil sinon renvoie 0
   ...
   SELECT   NVL(SUM (facture.montant)
          ...
          ,qttc_global.numgar
          ,facture.mregl
     INTO loc_montant, loc_numgar, loc_mregl
