CREATE FUNCTION ARTHUS."F_QTTC_DU_D" (a_idadhesion IN NUMBER, a_debut IN DATE, a_NbAnnee IN NUMBER DEFAULT 0)
   RETURN NUMBER
AS
   loc_montant   NUMBER;
BEGIN
   SELECT SUM (qttc_global.mt_ttc_d) - SUM (NVL (qttc_global.mt_affec_d, 0))
     INTO loc_montant
     FROM qttc_global
    WHERE qttc_global.idadhesion = a_idadhesion
      AND NVL (qttc_global.mt_affec_d, 0) < qttc_global.mt_ttc_d
      ...
      AND ((NVL(a_NbAnnee,0) = 0 AND debut < a_debut AND type_qttc != 3) OR -- avant la date de début
          (NVL(a_NbAnnee,0) > 0 AND debut >= ADD_MONTHS(TRUNC(a_debut, 'Y'), -12*(a_NbAnnee-1)) AND fin <= ADD_MONTHS(TRUNC(a_debut, 'Y'), 12) - 1) OR -- pour un nombre d'année (avant la date de début)
