CREATE PROCEDURE ARTHUS.P_REPRISE_SIN_PREV (p_ref in varchar2,
							 p_date date,
...
BEGIN
	...
	BEGIN
		...
		WHERE  s.ref_ext_1 = p_ref --s.nosin =p_ref
	...
	EXCEPTION
		WHEN NO_DATA_FOUND THEN dbms_output.put_line('Erreur aucune garantie trouvé contrat :'||v_numgar); RAISE exc_fin;
		WHEN OTHERS THEN dbms_output.put_line('Erreur adhesion garantie :'||v_numgar);RAISE exc_fin;
	END;
  ...
  INSERT INTO ADHESION (NUMINDIV,NUMGAR,NUMFOR,DATAPLI,DATPER,RANG,ETAT,
	...
	IF p_date IS NOT NULL THEN
		...
		EXCEPTION
			WHEN NO_DATA_FOUND THEN dbms_output.put_line('Erreur aucun arret trouvé nosin :'||v_nosin||' date:'||p_date-1); RAISE exc_fin;
			WHEN OTHERS THEN dbms_output.put_line('Erreur arret  nosin :'||v_nosin||' date:'||p_date-1);RAISE exc_fin;
		END;
    ...
    UPDATE VAL_VARIABLE SET VALEUR =0 WHERE IDVARIABLE = F_FIND_VAR('FORC_FIC') AND ETENDUE = 15 AND clef =v_nosin and VALEUR is null; --forcage à non
      ...
      UPDATE VAL_VARIABLE SET VALEUR =replace(p_salbrut,',','.') WHERE IDVARIABLE = F_FIND_VAR('SAL1') AND ETENDUE = 15 AND clef =v_nosin ;
      ...
      UPDATE VAL_VARIABLE SET VALEUR =p_nbenf WHERE IDVARIABLE = F_FIND_VAR('NBENFGAR') AND ETENDUE = 15 AND clef =v_nosin ;
