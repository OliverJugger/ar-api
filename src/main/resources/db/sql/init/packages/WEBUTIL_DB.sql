CREATE OR REPLACE PACKAGE ARTHUS.WEBUTIL_DB AUTHID CURRENT_USER AS
...
CREATE OR REPLACE PACKAGE BODY ARTHUS.WEBUTIL_DB AS
  m_binaryData   BLOB;
  ...
  BEGIN
    ...
    m_blobTable  := sys.dbms_assert.qualified_sql_name(blobTable);
    m_blobColumn := sys.dbms_assert.qualified_sql_name(blobColumn);
    m_blobWhere  := blobWhere;
    m_mode       := upper(openMode);

    if chunkSize is not null then
      if chunkSize > 23829 then
        MAX_READ_BYTES := 23829;
      else
