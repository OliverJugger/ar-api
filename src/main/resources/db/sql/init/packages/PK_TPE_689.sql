CREATE OR REPLACE PACKAGE ARTHUS.PK_TPE_689 AS
...
CREATE OR REPLACE PACKAGE BODY ARTHUS.PK_TPE_689 AS
-- Chaine de reconnaissance SCCS
-- %W%  %E%
-- -- CONSTANTES PRIVEES ------------------------------------------------------
-- Aucune
...
--
   g_prefixe          VARCHAR2 (11)                     := 'pk_tpe_689.';
   ...
   BEGIN
      ...
      EXCEPTION
         ...
         THEN
          ...
          --
/*    Vincent 16/10/2006 : table inexistante en V6REF et V6H  : à vérifier
          DELETE sntrprt_cetip
          WHERE   numremise = I_numremise;
         G_msg_adm      := 'Nombre de lignes supprimées dans sntrprt_cetip '||TO_CHAR(SQL%ROWCOUNT);
         P_INS_journal;
*/
          --
                  IF l_nature = 2
   ...
   BEGIN
      ...
      FOR encours IN
          (SELECT   numremise_import
              FROM suivi_fact_tpe s
             WHERE codevefac = 10 AND 
                   numremise_import NOT IN (
...
----------------------------------------------------
         ...
         --
         -- Information sur le remises pas encore calculées (même partiellement)
         --
            ...
            EXCEPTION
               ...
               THEN
                  g_msg_adm :=
                     ...
                     || TO_CHAR (p_numremise_in);
                  pk_trace.p_ins_journal_adm ('pk_tpe_689.p_export_acc_rej',
                                              1,
                                              0,
                                              g_msg_adm,
                                              SYSDATE,
...
--
  ...
  --
/* VCR 27/12/2006 l_repertoire obtenu par I_repertoire directement dans la BA21 */
/*  SELECT repertoire
  INTO l_repertoire
  FROM typ_batch
  WHERE batchid = I_traitement;
  IF l_repertoire IS NULL THEN
...
--CTT 09/11/2005 Mise en phase avec document SINTIA Alimentation 900 Version 3.0
                  ...
                  THEN
                     ...
                     -- stockees dans la table stock_entite_norm_900 lorsqu'on a atteint l'entite 209
...
--                                          SUBSTR ( rec.entite, 98, 31 ) );
                     ...
                     l_count_010 := 0;
                  --
                  -- On reformate chaque entite norme 689 en norme 900
                  --
                  ELSIF rec.cod_entite = '070'
                  THEN
                     l_070 := rec.entite;
                  ...
                  THEN
                     l_120 :=
                           '12005'
                        || SUBSTR (rec.entite, 6, 7)
                        || SUBSTR (rec.entite, 15, 45);
                  ELSIF rec.cod_entite = '025'
                  THEN
...
-- G_nom_traitement  := G_prefixe||'p_forcage_facture';
-- G_max_msg         := 1;
-- G_session         := 1;
-- G_idligne         := F_max_idligne(I_session => G_session);
-- G_niv_msg         := 1;
-- G_msg_adm         := 'Forcage le ' ||TO_CHAR(Sysdate, 'dd/mm/yyyy hh24:mi')||' Rem : <'||I_remise||'> Idfactpe :< '||I_idfactpe||'> Util :< '||I_forcage||'>';
...
-- *******************************************************************************************************************************************************************************
   ...
   BEGIN
      ...
      g_msg_adm :=
            'PK_TPE_689.P_SINISTRE_ACCEPTE - Mise à jour : '
         || TO_CHAR (SQL%ROWCOUNT);
      p_ins_journal;
