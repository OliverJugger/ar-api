CREATE OR REPLACE PACKAGE ARTHUS.pk_ex99t AS
--
PROCEDURE P_ex99t(
		 I_date_couv_ad		IN	Varchar2 default null,
		 I_categorie_deb	IN	Varchar2 default null,
		 I_categorie_fin	IN	Varchar2 default null,
...
CREATE OR REPLACE PACKAGE BODY ARTHUS.pk_ex99t AS
...
G_nom_traitement	Constant journal_adm.nom_traitement%TYPE default 'pk_ex99t';
...
--@curs
--
CURSOR c_info (I_date_couv_ad varchar2, I_categorie_deb varchar2, I_categorie_fin varchar2) IS
	...
	FROM	indvs,libelle c,adhe_cntrt
	...
	AND	exists (select 1
				from val_variable,
					val_variable a
			where val_variable.idvariable=9
			and val_variable.clef=a.clef
			and val_variable.etendue=13
			and val_variable.clef=adhe_cntrt.idadhesion
			and nvl(val_variable.fin,e2d(I_date_couv_ad))
				>=
				e2d(I_date_couv_ad)
			and a.idvariable=19
			and a.etendue=13
			and a.valeur between nvl(I_categorie_deb,a.valeur)
						 and nvl(I_categorie_fin,nvl(I_categorie_deb,a.valeur))
			and nvl(a.fin,e2d(I_date_couv_ad))>=e2d(I_date_couv_ad)
			and val_variable.valeur=1)
	ORDER BY indvs.nom;
--
----------------------------------------------------------------------------
--
...
--
	...
	P_INS_journal;
--
	FOR rec_info in c_info(I_date_couv_ad, I_categorie_deb, I_categorie_fin)
	LOOP
...
EXCEPTION WHEN OTHERS THEN
		G_niv_msg	:= 0;
		G_msg_adm	:= 'PK_ex99t - '||SUBSTR(SQLERRM(SQLCODE),1,128);
