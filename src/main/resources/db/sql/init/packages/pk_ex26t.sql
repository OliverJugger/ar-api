CREATE OR REPLACE PACKAGE ARTHUS.pk_ex26t AS
--
PROCEDURE P_ex26t(
...
CREATE OR REPLACE PACKAGE BODY ARTHUS.pk_ex26t AS
-- Chaine de reconnaissance SCCS
-- %W%	%E%
...
--
G_suffixe_fich_ex26t		Varchar2(10);
Nom_fich_ex26t   			Varchar2(30);
--
ex26_ligne			        Varchar2(3500);
--
G_proc			VARCHAR2(80);
-- Variables de P_INS_journal
G_nom_traitement	Constant journal_adm.nom_traitement%TYPE default 'pk_ex26t';
...
BEGIN
	...
	O_found         := 1;
	G_erreur        := Null;
	--Ctt 20/09/07 : Initialisation à partir des données reçues en paramètre (idem pk_ex25t)
	...
	END IF;
	--
	-- Ouverture du package pk_ext_assures.sql
	--
	G_trouve		:= 0;
	--
	LOOP
		pk_ext_assures.P_TRAITE_adhesion (
...
EXCEPTION
		WHEN NO_DATA_FOUND THEN
			...
			ELSE
				G_niv_msg	:= 0;
				G_msg_adm	:= 'PK_EX26T - '||SUBSTR(SQLERRM(SQLCODE),1,128);
...
--
                g_niv_msg     := 3;
                g_msg_adm     := 'ligne ex26 - '||'G_attest : ';
                p_ins_journal;
--
		UTL_FILE.PUT_LINE(F_TXT,ex26_ligne);
--
Exception
...
--
	...
	--
		G_datejour	:= To_char(Sysdate, 'DDMMYYYY');
       	G_suffixe_fich_ex26t	:= nvl(to_char(G_datejour),'0');
        Nom_fich_ex26t			:= 'EX26T_'||G_suffixe_fich_ex26t||'.txt';
		--
		-- EXPORT est le nom d'une Directory definie dans Oracle
		--
		F_TXT					:= UTL_FILE.FOPEN('EXPORT',Nom_fich_ex26t,'W',32767);
	--
Exception
...
------------
	...
	INSERT	INTO 	lib_edition	(numedit, editlib)
		VALUES			(G_session, ('Generation fichier EX26T'||G_datejour));
--
	G_niv_msg	:= 1;
