...
@Component
public class DossierSinistreNumeroAssureContainsRule implements DossierSinistreRule {
	...
	public boolean matches(DossierSinistreSearchCriteria criteria) {
		return Optional.ofNullable(criteria)
				.map(DossierSinistreSearchCriteria::getNumeroAssureContains)
				.map(Objects::nonNull)
				.orElse(false);
	}
    ...
    public Predicate apply(Root<DossierSinistre> root, CriteriaQuery<?> query, CriteriaBuilder builder, DossierSinistreSearchCriteria criteria) {
		...
		return builder.like(
					individu.get("numassu").as(String.class),
					'%' + criteria.getNumeroAssureContains() + '%');
    }
}