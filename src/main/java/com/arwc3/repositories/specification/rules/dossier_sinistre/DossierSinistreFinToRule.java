...
@Component
public class DossierSinistreFinToRule implements DossierSinistreRule {
    ...
    public boolean matches(DossierSinistreSearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(DossierSinistreSearchCriteria::getFinTo)
    			.map(Objects::nonNull)
    			.orElse(false);
    }
    ...
    public Predicate apply(Root<DossierSinistre> root, CriteriaBuilder builder, DossierSinistreSearchCriteria criteria) {
		return builder.lessThanOrEqualTo(root.get("fin"), criteria.getFinTo().plusDays(1).atStartOfDay().minusNanos(1));
    }
}