...
@Component
public class DossierSinistrePrenomIndividuContainsRule implements DossierSinistreRule {
	...
	public boolean matches(DossierSinistreSearchCriteria criteria) {
		return Optional.ofNullable(criteria)
				.map(DossierSinistreSearchCriteria::getPrenomIndividuContains)
				.map(Objects::nonNull)
				.orElse(false);
	}
    ...
    public Predicate apply(Root<DossierSinistre> root, CriteriaBuilder builder, DossierSinistreSearchCriteria criteria) {
		...
		return builder.like(
					builder.lower(individu.get("prenom")),
					'%' + criteria.getPrenomIndividuContains().toLowerCase() + '%');
    }
}