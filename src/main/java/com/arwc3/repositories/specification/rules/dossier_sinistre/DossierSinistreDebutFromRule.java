...
@Component
public class DossierSinistreDebutFromRule implements DossierSinistreRule {
    ...
    public boolean matches(DossierSinistreSearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(DossierSinistreSearchCriteria::getDebutFrom)
    			.map(Objects::nonNull)
    			.orElse(false);
    }
    ...
    public Predicate apply(Root<DossierSinistre> root, CriteriaBuilder builder, DossierSinistreSearchCriteria criteria) {
		return builder.greaterThanOrEqualTo(root.get("debut"), criteria.getDebutFrom().atStartOfDay());
    }
}