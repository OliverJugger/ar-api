...
@Log4j2
@Component
public class SchedulerInvestran {

	// Default value will be false
	@Value("${epbs.scheduler.navs.investran.enable:false}")
	private boolean isEnabledScheduler;
	
	@Value("${epbs.scheduler.navs.investran.execution.date}")
	@DateTimeFormat(pattern = "dd-MM-yyyy")
	private LocalDate executionDate;

	@Value("${epbs.scheduler.navs.investran.user}")
	private String tecUser;

	@Autowired
	private SubFundServiceV2 subFundServiceV2;

	@Scheduled(cron = "${epbs.scheduler.navs.investran.cron}")
    @SchedulerLock(name = "RoutineSynchroNavsInvestran", lockAtLeastFor = "10m", lockAtMostFor = "10m")
	public void executeSynchroAuditJob() {
		log.info("Scheduler.RoutineSynchroNavsInvestran start");
