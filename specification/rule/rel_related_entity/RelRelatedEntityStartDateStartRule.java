...
@Component
public class RelRelatedEntityStartDateStartRule implements RelRelatedEntityRule {
    ...
    public boolean matches(RelRelatedEntitySearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(RelRelatedEntitySearchCriteria::getStartDateStart)
    			.map(Objects::nonNull)
    			.orElse(false);
    }
    ...
    public Predicate apply(Root<RelRelatedEntity> root, CriteriaBuilder builder, RelRelatedEntitySearchCriteria criteria) {
        return  builder.greaterThanOrEqualTo(root.get(RelRelatedEntityRuleFields.START_DATE_FIELD_NAME), criteria.getStartDateStart());
    }
}
