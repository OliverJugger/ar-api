...
@Component
public class RelRelatedEntityEndDateStartRule implements RelRelatedEntityRule {
    ...
    public boolean matches(RelRelatedEntitySearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(RelRelatedEntitySearchCriteria::getEndDateStart)
    			.map(Objects::nonNull)
    			.orElse(false);
    }
    ...
    public Predicate apply(Root<RelRelatedEntity> root, CriteriaBuilder builder, RelRelatedEntitySearchCriteria criteria) {
        return  builder.greaterThanOrEqualTo(root.get(RelRelatedEntityRuleFields.END_DATE_FIELD_NAME), criteria.getEndDateStart());
    }
}
