...
@Component
public class RelRelatedEntityStartDateEndRule implements RelRelatedEntityRule {
    ...
    public boolean matches(RelRelatedEntitySearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(RelRelatedEntitySearchCriteria::getStartDateEnd)
    			.map(Objects::nonNull)
    			.orElse(false);
    }
    ...
    public Predicate apply(Root<RelRelatedEntity> root, CriteriaBuilder builder, RelRelatedEntitySearchCriteria criteria) {
        return  builder.lessThanOrEqualTo(root.get(RelRelatedEntityRuleFields.START_DATE_FIELD_NAME), criteria.getStartDateEnd());
    }
}
