...
@Component
public class RelRelatedIndividualStartDateStartRuleV2 implements RelRelatedIndividualRuleV2 {
    ...
    public boolean matches(RelRelatedIndividualSearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(RelRelatedIndividualSearchCriteria::getStartDateStart)
    			.map(Objects::nonNull)
    			.orElse(false);
    }
    ...
    public Predicate apply(Root<RelRelatedIndividualV2> root, CriteriaBuilder builder, RelRelatedIndividualSearchCriteria criteria) {
        return  builder.greaterThanOrEqualTo(root.get(RelRelatedIndividualRuleFields.START_DATE_FIELD_NAME), criteria.getStartDateStart());
    }
}
