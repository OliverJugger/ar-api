...
@Component
public class RelRelatedIndividualStartDateEndRuleV2 implements RelRelatedIndividualRuleV2 {
    ...
    public boolean matches(RelRelatedIndividualSearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(RelRelatedIndividualSearchCriteria::getStartDateEnd)
    			.map(Objects::nonNull)
    			.orElse(false);
    }
    ...
    public Predicate apply(Root<RelRelatedIndividualV2> root, CriteriaBuilder builder, RelRelatedIndividualSearchCriteria criteria) {
        return  builder.lessThanOrEqualTo(root.get(RelRelatedIndividualRuleFields.START_DATE_FIELD_NAME), criteria.getStartDateEnd());
    }
}
