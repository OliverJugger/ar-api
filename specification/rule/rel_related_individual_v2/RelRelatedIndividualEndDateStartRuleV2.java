...
@Component
public class RelRelatedIndividualEndDateStartRuleV2 implements RelRelatedIndividualRuleV2 {
    ...
    public boolean matches(RelRelatedIndividualSearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(RelRelatedIndividualSearchCriteria::getEndDateStart)
    			.map(Objects::nonNull)
    			.orElse(false);
    }
    ...
    public Predicate apply(Root<RelRelatedIndividualV2> root, CriteriaBuilder builder, RelRelatedIndividualSearchCriteria criteria) {
        return  builder.greaterThanOrEqualTo(root.get(RelRelatedIndividualRuleFields.END_DATE_FIELD_NAME), criteria.getEndDateStart());
    }
}
