...
@Component
public class RelRelatedIndividualLegalPersonIdRuleV2 implements RelRelatedIndividualRuleV2 {
	...
	public Predicate apply(Root<RelRelatedIndividualV2> root, CriteriaBuilder builder, RelRelatedIndividualSearchCriteria criteria) {
		Join<LegalPerson, RelLegalPersonNaturalPersonV2> legalPersonNaturalPerson = root
				.join(RelRelatedIndividualRuleFields.REL_LEGAL_PERSON_RELATED_INDIVIDUAL_FIELD_NAME)
				.join(RelRelatedIndividualRuleFields.REL_LEGAL_PERSON_NATURAL_PERSON_FIELD_NAME)
				.join(RelRelatedIndividualRuleFields.LEGAL_PERSON_FIELD_NAME);
		
		return builder.equal(legalPersonNaturalPerson.get(RelRelatedIndividualRuleFields.ID_FIELD_NAME), criteria.getLegalPersonId());
	}
