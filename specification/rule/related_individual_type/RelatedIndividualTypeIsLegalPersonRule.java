...
@Component
public class RelatedIndividualTypeIsLegalPersonRule implements RelatedIndividualTypeRule {
	private static final long serialVersionUID = 1L;

	@Override
    public boolean matches(RelatedIndividualTypeSearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(RelatedIndividualTypeSearchCriteria::getIsLegalPerson)
    ...
    public Predicate apply(Root<RelatedIndividualType> root, CriteriaBuilder builder, RelatedIndividualTypeSearchCriteria criteria) {
        return builder.equal(
        		root.get("isLegalPerson"), 
        		criteria.getIsLegalPerson().booleanValue());
    }
}
