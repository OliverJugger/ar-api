...
@Component
public class RelatedEntityTypeIsOnboardingRule implements RelatedEntityTypeRule {
	private static final long serialVersionUID = 1L;

	@Override
    public boolean matches(RelatedEntityTypeSearchCriteria criteria) {
    	return Optional.ofNullable(criteria)
    			.map(RelatedEntityTypeSearchCriteria::getIsOnboarding)
    ...
    public Predicate apply(Root<RelatedEntityType> root, CriteriaBuilder builder, RelatedEntityTypeSearchCriteria criteria) {
        return builder.equal(
        		root.get("isOnboarding"), 
        		criteria.getIsOnboarding());
    }
}
